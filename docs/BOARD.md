...
- **CI on push / PR** — `.github/workflows/ci.yml`: Node regression + board parser + codex bootstrap + Python suites (room_signed, room_claim, dm, security_fixes) + MCP smoke, all behind `pip install -r tests/requirements.txt`. Server log uploaded on failure.
